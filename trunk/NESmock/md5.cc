...
/* Translate Devine's concepts to openssl concepts */
typedef md5_context MD5_CTX;
#define MD5_DIGEST_LENGTH 16
#define MD5_Init(cp) md5_starts(cp)
#define MD5_Update(cp,s,l) md5_update(cp,(const uint8*)s,l)
#define MD5_Final(b,cp) md5_finish(cp,(uint8*)b)
...
const std::string MD5sum(const std::string& data)
{
    ...
    return std::string((char*)Buf, MD5_DIGEST_LENGTH);
}

/* Returns a 16-byte binary string in lowercase. */
const std::string MD5sumHex(const std::string& data)
{
    std::string md5 = MD5sum(data);
    char Buf[MD5_DIGEST_LENGTH*2+10];
    for(unsigned a=0; a<MD5_DIGEST_LENGTH; ++a)
    {
        if(a >= md5.size()) break;
        std::sprintf(Buf+a*2, "%02x", (unsigned char)md5[a]);
    }
    return std::string(Buf);
}

/* Returns a 16-byte binary string in uppercase. */
