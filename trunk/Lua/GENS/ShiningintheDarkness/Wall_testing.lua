...
Wachunga = true;
Combo = io.open("C:\\SitD_Wall.txt", "w");
 

local buttonmap = {[1]='up',[2]='down',[4]='left',[8]='right',[16]='A',[32]='B',[64]='C',[128]='start'} 


	memory.writeword(Z_Atk+6,25);
	memory.writeword(Z_Atk,28);
...
for RNGc = 8023,65535,1 do
    ...
    K_ChngL = 0;
    if M_Chng == 0 and P_Chng == 0 and memory.readword(K_Hp) == 0 then    
         outs = string.format('%d	%d		%d\n', RNGc, Z_Chng, movie.framecount());
    	 Combo:write(outs);   
    	 print(outs);   
    end;
end;
