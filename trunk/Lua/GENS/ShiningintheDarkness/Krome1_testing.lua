...
Combo = io.open("C:\\SitD_KromeNew.txt", "w");
...
for RNGc = 7000,65535,1 do
	...
	while(gens.lagged()) do  	    
		...
		gens.emulateframeinvisible();    	  		 
	     ...
	     gens.emulateframeinvisible();
	     gens.emulateframeinvisible(); 	
     while(gens.lagged() and memory.readword(P_Hp) > 0) do  	    
		gens.emulateframeinvisible();	    		
	end	 
    ...
    if memory.readbyte(Sleep) == 4  and  memory.readword(0xFF3354) == 5 and P_Chng == 0 and Z_Chng == 0 then	
	  	outs = string.format('%d	%d	%d	%d	%d\n', RNGc, K_Chng, Z_Chng, mov, memory.readword(RNG));
      	Combo:write(outs);   
      	print(outs);       
    end;
end;
