...
Z_Weight = {.8,    0,  3,  1.5,    0,  .2,  .1};
P_Weight = { 1,    0,  0,  -.3,   0,   .6,  2};
		...
		for j = 1,7,1 do
			...
			for bit,button in pairs(buttonmap) do
  			...
  			while (memory.readword(Hp+offset) == current[1]) do 
  				gui.text(1,80,string.format('Last Score: %f',currscore));
  				gui.text(1,90,string.format('Max Score: %f',Score));
			...
			if currscore > Score and memory.readbyte(0xFF1701) == 0x6C then
				Score = currscore;
...
if adv > 0 then
	for t = 1,adv,1 do
			...
			for bit,button in pairs(buttonmap) do
  			...
  			for i=1,5,1 do
				gui.text(1,80,string.format('Score: %d',Score));			
				gens.frameadvance();
			end;
