...
int main() {
    ...
    DWORD result = GetModuleFileName(GetModuleHandle(NULL), 
                                     &locale_dir[0], locale_dir.size() - 1);
    ...
    locale_dir.resize(locale_dir.size() + (sizeof(dir) / sizeof(*dir)) + 1);
...
#if defined(UNICODE) || defined(_UNICODE)
    _putws(L"UNICODE");
    if (wbindtextdomain(PACKAGE_NAME, &locale_dir[0]) == NULL) {
        return 1;
    }
