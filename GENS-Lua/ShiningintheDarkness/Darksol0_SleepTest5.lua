...
Combo = io.open("C:\\SitD_DS_F2-3.txt", "w");
...
for Cnt= 0,65536,1 do
	...
	if  memory.readbyte(E_Sleep)== 2 then
	      while (gens.lagged() and memory.readbyte(E_Sleep)== 2) do
				gens.emulateframeinvisible();
		end;		    
	K_Chng = K_Start - memory.readword(K_Hp);	
	if  memory.readbyte(E_Sleep)== 2 and  memory.readword(0xFF3354) == 5 and K_Chng > 200 then
